...
sub search {
    ...
    my ( $topicNames, $hits, $msg ) =
      $db->dbQuery( $params{query}, undef, $sort, $params{reverse} );
    ...
    foreach my $topic (@$topicNames) {
        ...
        foreach my $columnName (@selectedColumns) {
           ...
           # required for older JQueryPlugins
                ...
                $cell =
                  $fieldDef->renderForDisplay( '$value', $cell, undef,
                    $params{web}, $topic );
                $cell =
                  Foswiki::Func::expandCommonVariables( $cell, $topic,
                    $params{web} );
