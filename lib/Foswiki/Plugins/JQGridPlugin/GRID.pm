...
sub new {
    ...
    $Foswiki::cfg{JQGridPlugin}{Connector}{search} =
      'Foswiki::Plugins::JQGridPlugin::SearchConnector'
      unless defined $Foswiki::cfg{JQGridPlugin}{Connector}{search};
    $Foswiki::cfg{JQGridPlugin}{Connector}{dbcache} =
      'Foswiki::Plugins::JQGridPlugin::DBCacheConnector'
      unless defined $Foswiki::cfg{JQGridPlugin}{Connector}{dbcache};
    $Foswiki::cfg{JQGridPlugin}{Connector}{solr} =
      'Foswiki::Plugins::JQGridPlugin::SolrConnector'
    ...
    my $this = bless(
        $class->SUPER::new(
            ...
            dependencies  => [
                'ui',        'metadata',
                'livequery', 'JQUERYPLUGIN::THEME',
                'JQUERYPLUGIN::GRID::LANG'
            ],
        ),
        $class
    );
...
#print STDERR "called handleGrid(".$params->stringify."), topic=$topic, web=$web\n";
    ...
    my $gridId = $params->{id}
      || "jqGrid" . Foswiki::Plugins::JQueryPlugin::Plugins::getRandom();
    my $pagerId =
      "jqGridPager" . Foswiki::Plugins::JQueryPlugin::Plugins::getRandom();
...
# TODO: search configuration - see http://www.trirand.com/jqgridwiki/doku.php?id=wiki:search_config
            ...
            if ($formatter) {
                ...
                if ( $fieldName =~
                    /^(Date|Changed|Modified|info.date|info.createdate)$/ )
                {
                    push @colModel, "formatter:'date'";
                    push @colModel,
"formatoptions: {srcformat: 's', newformat: 'd M Y - H:i'}";
...
EOT
            ...
            if ( $theEdit eq 'on' ) {
                if ( $fieldName =~
/^(Changed|Modified|Author|info.date|info.author|Topic|topic)$/
                  )
                {
        ...
        $theConnector = $Foswiki::cfg{JQGridPlugin}{DefaultConnector}
          unless defined $theConnector;
        my ( $connectorWeb, $connectorTopic ) =
          Foswiki::Func::normalizeWebTopicName( $baseWeb, $theConnector );
        if ( Foswiki::Func::topicExists( $connectorWeb, $connectorTopic ) ) {
            ...
            if ( defined $Foswiki::cfg{JQGridPlugin}{Connector}{$theConnector} )
            {
                ...
                throw Error::Simple("unknown grid connector $theConnector")
                  ;    # SMELL: where's the catch
            }
        }
...
HERE
        ...
        Foswiki::Func::addToZone(
            'script',    "JQUERYPLUGIN::GRID::$gridId",
            $jsTemplate, 'JQUERYPLUGIN::GRID'
        );
