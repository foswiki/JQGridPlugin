...
HERE
    ...
    foreach my $columnName (@selectedFields) {
        ...
        elsif ( $propertyName =~ /^[a-zA-Z_]+$/ )
        { # SMELL: should check if this is a defined formfield consulting the DataForm definition
            $cell .= '$formfield(' . $propertyName . ')';
        }
        else {
            $cell .=
              '$percntQUERY{\"\'$web.$topic\'/' . $propertyName . '\"}$percnt';
        }
        $tml .=
            '<cell name=\"'
          . $columnName
