...
sub restHandleSearch {
    ...
    my $columns = Foswiki::Plugins::JQGridPlugin::Connector::urlDecode(
        $request->param('columns') || '' );
    ...
    my $solrResponse = $searcher->doSearch(
        $query,
        {
            web     => $web,
            query   => $query,
...
HERE
    ...
    for my $doc ( $solrResponse->docs ) {
        ...
        foreach my $columnName (@selectedColumns) {
            ...
            @values = $doc->values_for( "field_" . $propertyName . "_s" )
              unless @values;
            @values = $doc->values_for( "field_" . $propertyName . "_lst" )
              unless @values;
            @values = $doc->values_for( "field_" . $propertyName . "_dt" )
              unless @values;
            next unless @values;
           ...
           # required for older JQueryPlugins
                ...
                $value =
                  $fieldDef->renderForDisplay( '$value', $value, undef, $web,
                    $topic );
                $value =
                  Foswiki::Func::expandCommonVariables( $value, $topic, $web );
